    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $.get( "/system/role/initModuleData.do?roleId=${role.id}", function(data){
                ...
                zTreeObj.expandAll(true);  //true：全部展开，false：全部合上
            });
        });

        function submitCheckedNodes(){
            // getCheckedNodes  true 获取勾选的节点集合
            obj = zTreeObj.getCheckedNodes(true);
            console.log(obj);

            var moduleIds = "";
            for (var i=0; i<obj.length; i++){
                moduleIds = moduleIds + obj[i].id + ",";
            }
            console.log(moduleIds);  //201,206, 204, 205,
            moduleIds = moduleIds.substr(0, moduleIds.length-1);
            console.log(moduleIds);  //201,206, 204, 205

            $("#moduleIds").val(moduleIds);

            $("#icform").submit();   // updateRoleModule.do
        }
